"""Normalize an imported 3D asset and export it as GLB.

This script is embedded into the application binary and written to a
temporary file before being invoked:

Usage: blender -b -P normalize_to_glb.py -- <input_path> <output_path> <config_json>
...
def main():
    ...
    if len(argv) < 3:
        print("Usage: blender -b -P normalize_to_glb.py -- <input> <output> <config_json>")
        sys.exit(1)
...
def call_first_importer(candidates, path, ext):
    """Invoke the first registered operator from the candidate list.

    Import operator ids moved between releases (for example the legacy
    import_scene.obj was replaced by wm.obj_import), so try each candidate
    and report a clean error instead of an AttributeError.
    """
    for module_name, op_name in candidates:
        module = getattr(bpy.ops, module_name, None)
        op = getattr(module, op_name, None) if module is not None else None
        if op is None:
            continue
        try:
            op(filepath=path)
            ...
            continue
    print(f"[Normalizer] No available importer for {ext}: {path}")
    sys.exit(1)
...
def import_file(path):
    ...
    if ext == ".fbx":
        call_first_importer(
            [("import_scene", "fbx"), ("wm", "fbx_import")], path, ext
        )
    elif ext == ".obj":
        call_first_importer(
            [("wm", "obj_import"), ("import_scene", "obj")], path, ext
        )
    elif ext in (".glb", ".gltf"):
        call_first_importer(
            [("import_scene", "gltf"), ("wm", "gltf_import")], path, ext
        )
    elif ext == ".blend":
...
def correct_bone_axis_orientation():
    ...
    if not armatures:
        print("[Normalizer] No armature found, skipping bone axis correction")
        return
    ...
    print(f"[Normalizer] Bone axis correction applied to {len(armatures)} armature(s)")
...
def mark_leaf_bones_deform():
    ...
    for arm_obj in armatures:
        ...
        if leaf_count:
            print(f"[Normalizer] Preserved {leaf_count} leaf bone(s) as deform bones")

    print("[Normalizer] Leaf bone preservation complete")


def bake_all_animations():
    armatures = [obj for obj in bpy.context.scene.objects if obj.type == "ARMATURE"]
    if not armatures:
        print("[Normalizer] No armature found, skipping animation bake")
        return
    ...
    for arm_obj in armatures:
        ...
        baked_count += 1
        print(
            f"[Normalizer] Baked animation for '{arm_obj.name}' "
            f"(frames {frame_start}-{frame_end})"
        )

    if baked_count:
        print(f"[Normalizer] Baked {baked_count} animation(s)")
